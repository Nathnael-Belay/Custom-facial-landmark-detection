...
# in the Ibug dataset, each (x, y) coordinate maps to a specific
# facial feature. In order to train a dlib shape predictor on just a specific 
# landmark, in this case the eyes, we  must first define the integer indexes that
# belong to the eyes
...
for row in rows:
    # check to see of the current line has the (x, y)-coordinates for
    # the facial landmarks we are interested in
    parts = re.findall(PART, row)

    # if there is no information related to the coordinates of the facial
    ...
    # otherwise, there is annotation information that we must process
        ...
        # if the facial landmark name exists within the range of our indexes,
